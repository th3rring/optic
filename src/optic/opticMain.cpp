...
void usage(char *argv[]) {
  ...
  cout << "Modified by Thomas Herring\n";
  cout << "By releasing this code we imply no warranty as to its reliability\n";
  cout << "and its use is entirely at your own risk.\n\n";
  cout << "Usage: " << argv[0]
       << " [OPTIONS] domainfile problemfile [planfile, if -r or -y "
          "specified]\n\n";

  cout << "Options are: \n\n";
  cout << "\t-N\tDon't optimise solution quality (ignores preferences and "
          "costs);\n";
  cout << "\t-0\tAbstract out timed initial literals that represent recurrent "
          "windows;\n";
  cout << "\t-y\tPrint plan to file specified by planfile\n";
  ...
  cout << "\t"
       << "-c"
       << "\t\t"
       << "Enable the tie-breaking in RPG that favour actions that slot into "
          "the partial order earlier;\n";
  cout << "\t"
       << "-S"
       << "\t\t"
       << "Sort initial layer facts in RPG by availability order (only use if "
          "using -c);\n";
  cout << "\t"
       << "-m"
       << "\t\t"
       << "Disable the tie-breaking in search that favours plans with shorter "
          "makespans;\n";
  cout << "\t"
       << "-F"
       << "\t\t"
       << "Full FF helpful actions (rather than just those in the RP "
...
int main(int argc, char *argv[]) {
  ...
  while (argcount < argc && argv[argcount][0] == '-') {
    ...
    if (remainder == "citation") {
      ...
      cout << "If you are using the planner to optimise preferences or goals "
              "with time-dependent\n";
      cout << "collection costs, the citation for the paper describing OPTIC "
              "is:\n\n";
      cout << "@CONFERENCE{opticicaps,\n";
      cout << "\tauthor = \"J. Benton and A. J. Coles and A. I. Coles\",\n";
      cout << "\ttitle = \"Temporal Planning with Preferences and "
              "Time-Dependent Continuous Costs\",\n";
      cout << "\tbooktitle = \"Twenty Second International Conference on "
      ...
      cout << "If you are using the planner as an implementation of the `TIL "
              "Abstraction' extension\n";
      cout << "of POPF, the relevant paper is:\n\n";
      cout << "@CONFERENCE{lsfrpicaps,\n";
      cout << "\tauthor = \"K. Tierney, A. J. Coles, A. I. Coles, C. Kroer, A. "
              "Britt, and R. M. Jensen\",\n";
      cout << "\ttitle = \"Automated Planning for Liner Shipping Fleet "
              "Repositioning\",\n";
      cout << "\tbooktitle = \"Twenty Second International Conference on "
              "Automated Planning and Scheduling (ICAPS 12)\",\n";
      cout << "\tyear = \"2012\",\n";
      ...
      cout << "\tauthor = \"A. J. Coles and A. I. Coles and M. Fox and D. "
              "Long\",\n";
      cout << "\ttitle = \"Forward-Chaining Partial-Order Planning\",\n";
      cout << "\tbooktitle = \"Twentieth International Conference on Automated "
      ...
      case 'W': {
        ...
        if (Warg.empty()) {
          cerr << "Error: must specify weight after W, e.g. -W5, or specify a "
                  "weight and restart weight reduction, e.g. -W5,1\n";
          usage(argv);
          exit(1);
        }
        ...
        if (commaAt == string::npos) {
          ...
          if (!(conv >> FF::doubleU)) {
            cerr << "Error: must specify weight after W, e.g. -W5, or specify "
                    "a weight and restart weight reduction, e.g. -W5,1\n";
            usage(argv);
            exit(1);
          }
        } else {
          const string W = Warg.substr(0, commaAt);
          {
            ...
            if (!(conv >> FF::doubleU)) {
              cerr
                  << "Error: must specify weight after W, e.g. -W5, or specify "
                     "a weight and restart weight reduction, e.g. -W5,1\n";
              usage(argv);
              exit(1);
            }
  ...
  if (argcount + ((writePlanToFile || readInAPlan || debugPreprocessing)
                      ? expectFromHere + 1
                      : expectFromHere) >
      argc) {
    usage(argv);
  ...
  Globals::optimiseSolutionQuality =
      (Globals::optimiseSolutionQuality || postHocScheduleToMetric);
#endif
  ...
  if (Globals::optimiseSolutionQuality &&
      Globals::givenSolutionQualityDefined) {
    if (RPGBuilder::getMetric()) {
      cout << "Forcing the use of the given solution quality of "
           << Globals::givenSolutionQuality << endl;
      if (RPGBuilder::getMetric()->minimise) {
        Globals::bestSolutionQuality = (Globals::givenSolutionQuality == 0.0
                                            ? 0.0
                                            : -Globals::givenSolutionQuality);
      } else {
  ...
  if (spSoln) {
    ...
    for (int pass = 0; pass < 2; ++pass) {
      ...
      cout << "Saving plan to " << planfile << endl;

      std::ofstream out(planfile);
    ...
    // Commenting out to keep interaction simple
    /*     tms refReturn; */
    /*     times(&refReturn); */
    /*     double secs = ((double)refReturn.tms_utime +
     * (double)refReturn.tms_stime) / */
...
void split(const int &insAt, list<FFEvent>::iterator insStart,
           const list<FFEvent>::iterator &insItr,
...
list<FFEvent> *readPlan(char *filename) {
  ...
  for (int idebug = 0, i = 0; planItr != planEnd; ++planItr, ++i, ++idebug) {
    ...
    if (!currOp) {
      ...
      cout << "Exiting: step " << idebug
           << " in the input plan uses the action " << *(debugOp)
           << ", which the instantiation code in the planner does not "
              "recognise.\n";
      exit(1);
    }
    ...
    if (RPGBuilder::getRPGDEs(ID).empty()) { // non-durative action
      ...
      for (int pass = 0; pass < 2; ++pass) {
        if (pass)
          assert(startIdx >= 0);
        const double ts =
            (pass ? currStep->start_time + actDur : currStep->start_time);
        if (debug) {
        ...
        FFEvent toInsert = (pass ? FFEvent(currOp, startIdx, actDur, actDur)
                                 : FFEvent(currOp, actDur, actDur));
        toInsert.lpTimestamp = ts;
        toInsert.lpMinTimestamp = ts;
        ...
        for (; insItr != insEnd; ++insItr, ++insAt) {
          if (ts < insItr->lpTimestamp) {
            split(insAt, toReturn->begin(), insItr, insEnd);
            const list<FFEvent>::iterator dest =
                toReturn->insert(insItr, toInsert);
            if (pass) {
              startIsAt->pairWithStep = insAt;
              if (debug)
                cout << " putting at step " << insAt << ", pairing with "
                     << startIdx << "\n";
            } else {
              startIsAt = dest;
              startIdx = insAt;
  ...
  const vector<RPGBuilder::FakeTILAction *> &tils =
      RPGBuilder::getNonAbstractedTILVec();
